...
本文件为 Codex（以及任何兼容的 AI 编程助手）协作此项目时的指令。

## 项目概览

基于 LangChain/LangGraph 实现的最小版本的 Codex，用 Python 3.12 实现，可逐步演进、最终能本地化部署成可执行文件。
...
- **模型 Provider**：`mincc/llm.py` 中通过 `init_chat_model` 工厂支持 `Codex`、`openai`、`deepseek` 三种；DeepSeek 走 OpenAI 兼容接口（默认端点 `https://api.deepseek.com/v1`，可用 `LLM_BASE_URL` 覆盖）
...
## 其它
- 所有代码都要兼容不同的平台, 包含macos, windows, linux, unix
